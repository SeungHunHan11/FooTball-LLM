...
class Index_Builder:
    ...
    def __init__(self, model, tokenizer, data_dir, index_save_dir, use_content_type = 'All', batch_size = 32, retrieve_mode = 'sparse'):
        ...
        if self.use_content_type == "title":
            ...
            #self.doc_content = [item['title'] + item['contents'] for item in self.all_docs]
        ...
        if retrieve_mode != 'sparse':
            ...
            for idx, passage in enumerate(self.doc_content):
            ...
            # # unlist doc
            #self.corpus = [item for sublist in corpus for item in sublist]

            # unlist doc_id_ref
            #doc_id_ref = [item for sublist in doc_id_ref for item in sublist]

            self.doc_id_ref = np.array(doc_id_ref)
    ...
    def build_sparse_index(self):
        ...
        with open(os.path.join(self.index_save_dir, 'sparse','bm25_index.pickle'), 'wb') as index_file:
    ...
    def search(self, query, top_k: int = 20, searcher = None):
        ...
        if self.retrieve_mode == 'dense':
            ...
            scores = self.searcher.get_scores(query)
            top_k_list = np.argpartition(scores,-1*top_k)[-1*top_k:]
            top_k_indices = self.doc_id_ref[top_k_list]    
            ...
            #     raw_passage = self.corpus[idx]
            #     top_k_passage.append(raw_passage)

        return raw_reference_list #, top_k_passage
